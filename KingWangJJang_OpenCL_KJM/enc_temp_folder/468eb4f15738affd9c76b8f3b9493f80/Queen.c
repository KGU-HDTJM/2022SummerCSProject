...
void Add(int* a, int* b, int* result)
{
	...
	cl_mem SubW2 = SubBufferAlloc(WBuffers[0], sizeof(b), CLWriteBuf);

	clEnqueueWriteBuffer(CommandQueue, SubW1, CL_TRUE, 0, sizeof(int) * 4, a, 0, NULL, NULL);
	clEnqueueWriteBuffer(CommandQueue, SubW2, CL_TRUE, 0, sizeof(int) * 4, b, 0, NULL, NULL);
	clSetKernelArg(KernelCro, 0, sizeof(cl_mem), &SubW1);
	clSetKernelArg(KernelCro, 1, sizeof(cl_mem), &SubW2);
	clSetKernelArg(KernelCro, 2, sizeof(cl_mem), &SubR);
	globalSize = 1;
	localSize = 1;
	for (int i = 0; i < 4; i++)
	{
	...
	clEnqueueReadBuffer(CommandQueue, SubR, CL_TRUE, 0, sizeof(int) * 4, result, 0, NULL, NULL);
}
